...
func (seg *SegmentTree) find(targetL, targetR, val, l, r, i int) int {
	...
	// search right first (to get last occurrence)
	if targetR >= mid+1 {
...
func main() {
	nums := []int{2, 5, 4, 3}
	fmt.Println(longestBalanced(nums)) // Expected: 4

	nums2 := []int{3, 2, 2, 5, 4}
	fmt.Println(longestBalanced(nums2)) // Expected: 5

	nums3 := []int{1, 2, 3, 2}
	fmt.Println(longestBalanced(nums3)) // Expected: 3

	nums4 := []int{22, 36, 22}
	fmt.Println(longestBalanced(nums4)) // Expected: 0
}
